...
def get_user_inputs():
    ...
    mode_list = []
    while True:
        mode_input = input("Enter the modes to animate (press Enter once all modes are added): ")
        if mode_input == "":
            break
        else:
            mode_list.append(int(mode_input))
    td_input = input("Top down view? (y/n): ")
    td = False
    if td_input == "y":
        td = True
    elif td_input != "n":
        print("invalid answer. Top down view not set")
    return amp, mode_list, td
...
def animate(i):
    global amplitude
    qmodes = amplitude * np.sin(times[:, i])
    geo = adjust(dat.geometry, Q[:, vibModes], qmodes)
    build_mol_figure(geo)
...
Q = dat.mw_normal_modes[0][:, 6:]
masses = dat.masses[0]

phase = [0, np.pi/2]
vibFreqsAU = freqs[vibModes] * kToAU
totalTime = 4 * np.pi/vibFreqsAU[0]
steps = 100
times = np.zeros((len(vibModes), steps))
for i, freq in enumerate(vibFreqsAU):
    times[i] = freq * np.linspace(0, totalTime, steps) + phase[i]

fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
if top_down:
    ax.view_init(90, 90, 0)  # top-down view
init_geo = dat.geometry.reshape(nAtoms, 3)
build_mol_figure(init_geo)
...
# `interval` for `frames` frames per second
frames = 100
interval = 1000 / 100
ani = FuncAnimation(fig, animate, repeat=True, frames=range(steps), interval=interval)
